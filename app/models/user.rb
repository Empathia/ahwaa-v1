class User < ActiveRecord::Base
  ...
  has_many :recent_replies, :class_name => "Reply", :limit => 3, :group => :topic_id, :conditions => "as_anonymous = 0"
  ...
  scope :inactive, select('COUNT(replies.user_id) AS replies_count, COUNT(topic_requests.user_id) as topic_requests_count, COUNT(visited_topics.user_id) as visited_topics_count, users.*')\
                .joins("LEFT OUTER JOIN replies ON (replies.user_id = users.id AND replies.created_at > date_sub(users.created_at, interval 6 month))")\
                .joins("LEFT OUTER JOIN topic_requests ON (topic_requests.user_id = users.id AND topic_requests.created_at > date_sub(users.created_at, interval 6 month))")\
                .joins("LEFT OUTER JOIN visited_topics ON (visited_topics.user_id = users.id AND visited_topics.created_at > date_sub(users.created_at, interval 6 month))")\
