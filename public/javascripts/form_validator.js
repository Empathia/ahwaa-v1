$.fn.formValidator = function(options){
    return this.each(function(){   
        if(this.getAttribute('type')) {
            $(this).click(function(){  
                validateForm($(this).closest('form'), options);
            });
        }
        else{
            var _form = $(this);
            _form.submit(function(){  
                validateForm(_form, options);
            });            
        }
    });    
    ...
    function validateForm(_form, options){
         _form.find('input').each(function(e){
             ...
             error ? _input.addClass('error') && options.errors[type] && _input.next('.error').remove().end().after('<p class="error">' + options.errors[type] + '</p>') : _input.hasClass('error') && _input.removeClass('error');
         }); 
